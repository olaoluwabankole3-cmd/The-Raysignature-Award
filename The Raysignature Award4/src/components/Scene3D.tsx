...
export default function Scene3D({ page = "gala", onLoaded }: Scene3DProps) {
  ...
  useEffect(() => {
    ...
    .to(state, {
      trophyY: isMobile ? 0.05 : -0.4,
      trophyScale: isMobile ? 0.72 : 1.0,
      duration: 2.2,
      ease: "power3.out"
    })
    .to(state, {
      ...
      mm.add("(max-width: 1023px)", () => {
        const tl = gsap.timeline({
          scrollTrigger: {
            ...
            scrub: 1, // Uses identical smooth scrub inertia as desktop
            invalidateOnRefresh: true,
          },
        });

        // MOVE 1: Landing over the right-side AWARD Node on mobile winding path
        tl.to(state, {
          trophyX: 1.15,
          trophyY: 0.1,
          cameraZ: 8.0,
        ...
        // MOVE 2: Landing over the left-side AWARD Node on mobile winding path
        .to(state, {
          trophyX: -1.15,
          trophyY: -0.85,
          cameraZ: 8.0,
          trophyRotationY: Math.PI * 4.8,
          trophyRotationX: -0.05,
          lightIntensityGold: 22,
          lightIntensityRim: 30,
          trophyScale: 0.72,
        ...
        // MOVE 3: Centered for horizontal carousel showcase under mobile path
        .to(state, {
          trophyX: 0,
          trophyY: 1.3,
          cameraZ: 8.5,
          trophyScale: 0.54,
          trophyRotationY: Math.PI * 6.2,
          lightIntensityGold: 18,
          lightIntensityRim: 26,
          duration: 1,
          ease: "power2.inOut"
        })
        // MOVE 4: Ending centered beautifully at the footer countdown
        .to(state, {
          trophyX: 0,
          trophyY: 0.2,
          cameraZ: 7.2,
          trophyScale: 0.78,
          trophyRotationY: Math.PI * 8.0,
          trophyRotationX: 0.1,
          lightIntensityGold: 35,
          lightIntensityRim: 48,
          duration: 1,
          ease: "power1.out"
         });
      });
    ...
    const animate = () => {
      ...
      if (trophyGroupRef.current) {
        ...
        trophyGroupRef.current.position.y = state.trophyY + hoverOffset;
        trophyGroupRef.current.position.x = state.trophyX;
        ...
        trophyGroupRef.current.scale.setScalar(state.trophyScale);
      }
  ...
  useEffect(() => {
    ...
    const state = renderState.current;
    if (page === "voting" || page === "gallery") {
      ScrollTrigger.getAll().forEach((trigger) => trigger.disable());
      
      const isMobileLocal = window.innerWidth < 1024;
      gsap.to(state, {
        trophyX: isMobileLocal ? 0 : 2.3,
        trophyY: isMobileLocal ? 1.3 : -0.2,
        cameraZ: isMobileLocal ? 8.5 : 6.5,
        trophyScale: isMobileLocal ? 0.54 : 1.0,
        trophyRotationY: Math.PI * 3.2,
        trophyRotationX: 0.1,
